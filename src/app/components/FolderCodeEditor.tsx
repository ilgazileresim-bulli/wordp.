...
export default function FolderCodeEditor({ onBack }: { onBack: () => void }) {
  ...
  const loadDirectory = async (dir: FileSystemDirectoryHandle) => {
    ...
    setTerminalLines(prev => [...prev, `[${now}] ${dir.name} klasörü açıldı — ${loaded.length} dosya yüklendi.`]);
  };
  ...
  const saveFile = useCallback(async (name: string, content: string) => {
    ...
    try {
      ...
      setSaveStatus("saved");
      const now = new Date().toLocaleTimeString("tr-TR");
      setTerminalLines(prev => [...prev, `[${now}] ${name} kaydedildi ✓`]);
    } catch (e: any) {
      setSaveStatus("unsaved");
  ...
  const createNewFile = async () => {
    ...
    if (!name) { setNewFileError("Dosya adı boş olamaz."); return; }
    if (!SUPPORTED_EXT.some(e => name.endsWith(e))) {
      setNewFileError("Desteklenen uzantılar: " + SUPPORTED_EXT.join(", "));
      return;
    }
    if (allFiles.find(f => f.name === name)) { setNewFileError("Bu isimde dosya zaten var."); return; }
    if (!dirHandle) return;
    try {
      ...
      setNewFileName("");
      const now = new Date().toLocaleTimeString("tr-TR");
      setTerminalLines(prev => [...prev, `[${now}] ${name} oluşturuldu ✓`]);
    } catch (e: any) {
      setNewFileError("Dosya oluşturulamadı: " + e?.message);
    }
  };
  ...
  const runPreview = () => {
    ...
    setPreviewKey(k => k+1);
    const now = new Date().toLocaleTimeString("tr-TR");
    setTerminalLines(prev => [...prev, `[${now}] Proje derlendi, önizleme başlatıldı ✓`]);
  };
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-[#1e1e1e] text-white"
        ...
        <div className="text-center max-w-md px-6">
          ...
          <p className="text-[#555] text-xs mt-4">
            Klasördeki .html, .css, .js, .ts, .json ve daha fazla dosyayı okuyup düzenleyebilirsiniz.
          </p>

          <div className="mt-12 grid grid-cols-2 gap-3 text-left">
            {[
              {icon:<FileCode size={16} className="text-orange-400"/>, text:"HTML dosyaları"},
              {icon:<Braces size={16} className="text-blue-400"/>,    text:"CSS dosyaları"},
              {icon:<FileCode2 size={16} className="text-yellow-400"/>,text:"JS / TS dosyaları"},
    ...
    <div className="h-screen flex flex-col bg-[#1e1e1e] text-[#cccccc] overflow-hidden"
      ...
      <div className="flex items-center bg-[#323233] px-2 py-1 text-[12px] text-[#cccccc] shrink-0 gap-2 select-none">
        <div className="flex items-center gap-1.5 mr-2">
          <button onClick={onBack} title="Geri" className="w-3 h-3 rounded-full bg-[#ff5f57] hover:brightness-110 transition-all flex items-center justify-center group">
            <X size={7} className="opacity-0 group-hover:opacity-100 text-red-900" />
          </button>
          <button onClick={() => setPhase("pick")} title="Farklı klasör aç" className="w-3 h-3 rounded-full bg-[#ffbd2e] hover:brightness-110 transition-all flex items-center justify-center group">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-12 bg-[#333333] flex flex-col items-center py-2 gap-1 shrink-0 border-r border-[#252526] select-none">
          ...
          <button className="w-10 h-10 flex items-center justify-center text-[#858585] hover:text-[#cccccc]" title="Ayarlar"><Settings size={22}/></button>
        </div>
        ...
        <div className="w-56 bg-[#252526] flex flex-col shrink-0 border-r border-[#1e1e1e] overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto">
                {explorerOpen && (
                  <div className="pl-3">
                    {filteredFiles.length === 0 && (
                      <p className="text-[11px] text-[#555] px-2 py-2">Dosya yok.</p>
                    )}
                    {filteredFiles.map(file=>(
                      ...
                      >
                        ...
                        <button onClick={(e)=>deleteFile(file.name,e)} title="Sil" className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-red-500/20 rounded text-red-400 transition-all shrink-0">
                          <Trash2 size={10}/>
                        </button>
                      </div>
                    ))}
                ...
                {showNewFileDialog && (
                  <div className="mx-2 mt-1 p-2 bg-[#1e1e1e] border border-[#454545] rounded-lg">
                    <p className="text-[10px] text-[#858585] mb-1">Yeni dosya adı:</p>
                    <input
                      autoFocus
                      value={newFileName}
                    ...
                    <div className="flex gap-1 mt-1.5">
                      <button onClick={createNewFile} className="flex-1 text-[10px] bg-blue-600 hover:bg-blue-500 text-white rounded px-2 py-1 transition-all">Oluştur</button>
                      <button onClick={()=>{setShowNewFileDialog(false);setNewFileName("");setNewFileError("");}} className="flex-1 text-[10px] bg-[#3c3c3c] hover:bg-[#484848] text-[#cccccc] rounded px-2 py-1 transition-all">İptal</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {activePanel === "search" && (
            <div className="p-3 flex flex-col gap-2">
              <div className="text-[11px] font-bold text-[#bbbbbb] uppercase">Dosyalarda Ara</div>
              ...
              />
              <div className="text-[11px] text-[#555]">{filteredFiles.length} dosya bulundu.</div>
          ...
          {activePanel === "git" && (
            <div className="p-3 flex-1 overflow-y-auto">
              ...
              {unsavedFiles.length > 0 ? (
                <>
                  <div className="text-[11px] text-[#cccccc] mb-1">Değişiklikler ({unsavedFiles.length})</div>
                  {unsavedFiles.map(f=>(
              ...
              ) : <div className="text-[11px] text-[#555]">Değişiklik yok.</div>}
            </div>
          )}
            ...
            <div className="p-3 flex-1 overflow-y-auto">
              ...
              {["HTML Snippets","CSS IntelliSense","JS Helper","Live Preview","Auto Save"].map(ext=>(
                <div key={ext} className="text-[12px] text-[#cccccc] py-1.5 border-b border-[#333] flex items-center justify-between">
                  <span>{ext}</span><span className="text-[10px] text-emerald-400">✓ Aktif</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex items-center bg-[#252526] border-b border-[#1e1e1e] overflow-x-auto shrink-0 select-none">
            ...
            <button onClick={()=>setShowNewFileDialog(true)} className="px-3 py-2 text-[#858585] hover:text-[#cccccc] hover:bg-white/5 transition-all" title="Yeni Dosya"><Plus size={14}/></button>
          </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            {activeTab ? (
                ...
                <div className="flex items-center justify-between px-4 py-0.5 bg-[#1e1e1e] border-b border-[#252526] text-[11px] text-[#858585] select-none">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={()=>activeFile&&saveFile(activeFile.name,activeFile.content)}
                      className="flex items-center gap-1 px-2 py-0.5 hover:bg-white/10 rounded text-[10px] text-[#858585] hover:text-[#cccccc] transition-all" title="Kaydet (Ctrl+S)">
                      <Save size={11}/> Kaydet
                    </button>
                  </div>
                </div>
              ...
              <div className="flex-1 flex items-center justify-center bg-[#1e1e1e] text-[#555] flex-col gap-3">
                ...
                <p className="text-[13px]">Düzenlemek için sol taraftan bir dosya seçin.</p>
                <button onClick={()=>setShowNewFileDialog(true)}
                  ...
                  <Plus size={14}/> Yeni Dosya Oluştur
                </button>
              </div>
            )}
            ...
            {isPreviewOpen && (
              <div className={`flex flex-col ${isFullPreview?"w-full":"w-1/2"} border-l border-[#252526]`}>
                <div className="flex items-center justify-between px-3 py-1 bg-[#252526] border-b border-[#1e1e1e] text-[11px] text-[#cccccc] select-none">
                  <div className="flex items-center gap-2"><Monitor size={12} className="text-emerald-400"/><span>Canlı Önizleme</span></div>
                  <div className="flex items-center gap-1">
      ...
      <div className="flex items-center justify-between bg-[#007acc] text-white text-[11px] px-3 py-0.5 shrink-0 select-none">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="flex items-center gap-1 hover:bg-white/20 px-2 py-0.5 rounded transition-all"><ArrowLeft size={11}/> Geri</button>
        ...
        <div className="flex items-center gap-3">
          {activeTab && (
            <>
              <span>Ln {cursorInfo.line}, Sütun {cursorInfo.col}</span>
          ...
          <button onClick={runPreview} className="flex items-center gap-1 hover:bg-white/20 px-1.5 py-0.5 rounded transition-all"><Play size={11}/> Önizle</button>
          {activeFile&&!activeFile.saved&&(
            <button onClick={()=>saveFile(activeFile.name,activeFile.content)} className="flex items-center gap-1 hover:bg-white/20 px-1.5 py-0.5 rounded transition-all"><Save size={11}/> Kaydet</button>
          )}
        </div>
      </div>
    </div>
