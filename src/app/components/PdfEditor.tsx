...
export default function PdfEditor({
  ...
  const rotatePage = async () => {
    ...
    try {
      ...
      const loadingTask = pdfjsLib.getDocument({
        data: new Uint8Array(newBytes.slice(0)),
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const blob = new Blob([modifiedPdfBytes.buffer as ArrayBuffer], {
  ...
  const exportAsImage = () => {
    ...
    try {
      ...
      const baseName = fileName.toLowerCase().endsWith(".pdf") 
        ? fileName.slice(0, -4) 
        : fileName;
      link.download = `${baseName}_sayfa_${currentPage}.png`;
      link.click();
    } catch (err) {
  ...
  const redrawCanvas = () => {
    ...
    const drawAnnotation = (
      ...
      if (ann.type === "sign" && ann.signatureData) {
        ...
        context.drawImage(
          img,
          ann.x * viewport_width,
          ann.y * viewport_height,
          signWidth,
          signHeight,
        );
        ...
        if (selectedId && (ann as Annotation).id === selectedId) {
          ...
          context.lineWidth = 1;
          context.strokeRect(
            ann.x * viewport_width - 5,
            ann.y * viewport_height - 5,
            signWidth + 10,
            signHeight + 10,
          );
        }
      ...
      if (ann.type === "stamp") {
        ...
        if (selectedId && (ann as Annotation).id === selectedId) {
          ...
          context.strokeRect(
            px - padding - 5,
            py - fontSize - padding - 5,
            metrics.width + padding * 2 + 10,
            fontSize + padding * 2 + 10,
          );
        }
        return;
      }
      ...
      if (ann.type === "note") {
        ...
        lines.forEach((line, i) => {
          context.fillText(
            line,
            px + padding,
            py + padding + fontSize + i * fontSize * 1.3,
          );
        });
      ...
      if (ann.type === "text") {
        ...
        if (selectedId && (ann as Annotation).id === selectedId) {
          ...
          context.strokeRect(
            ...
            ann.y * viewport_height -
              (ann.fontSize || 16) * (viewport_width / 500),
            metrics.width + padding * 2,
      ...
      context.beginPath();
      context.moveTo(
        ann.points[0].x * viewport_width,
        ann.points[0].y * viewport_height,
      );
    ...
    const findClickedAnnotation = () => {
      return [...annotations].reverse().find((ann) => {
        ...
        return ann.points.some(
          (p) => Math.sqrt(Math.pow(p.x - x, 2) + Math.pow(p.y - y, 2)) < 0.02,
        );
      });
    };
    ...
    if (activeTool === "erase") {
      setAnnotations((prev) =>
        prev.filter((ann) => {
          ...
          return !ann.points.some(
            (p) =>
              Math.sqrt(Math.pow(p.x - x, 2) + Math.pow(p.y - y, 2)) < 0.02,
          );
        }),
      );
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-screen text-zinc-800 font-sans selection:bg-rose-500/30",
        isEmbedded ? "bg-transparent h-full" : "bg-[#e6e6e6]",
      )}
    >
      {/* Word Header (Blue) */}
      {!isEmbedded && (
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.toLowerCase()
                    ? "bg-[#f3f2f1] !text-[#2b579a] rounded-t-sm"
                    : "text-white",
                )}
              >
                {tab}
              ...
              <div className="flex flex-col items-center h-full min-w-[90px]">
                <div className="flex-1 flex items-center gap-2">
                  <button
                    ...
                    className={cn(
                      "p-2 rounded transition-all flex flex-col items-center gap-1 w-12",
                      activeTool === "select"
                        ? "bg-white shadow-md ring-2 ring-[#2b579a]"
                        : "hover:bg-white/60",
                    )}
                  >
                    <MousePointer2
                  ...
                  <div className="flex flex-col gap-0.5">
                    <button
                      ...
                      className="p-1 px-2 hover:bg-white/60 rounded text-[9px] flex items-center gap-2 text-zinc-800 font-bold"
                    >
                      <Upload
                        size={12}
                        className="text-[#2b579a]"
                    ...
                    <button
                      onClick={handleSave}
                      className="p-1 px-2 hover:bg-white/60 rounded text-[9px] flex items-center gap-2 !text-zinc-950 font-black"
                    >
                      <Download
                        size={14}
                        className="!text-[#2b579a]"
                ...
                <div className="flex-1 flex flex-col gap-1 justify-center">
                  <div className="flex items-center gap-1">
                    <div className="bg-white border border-[#dadada] rounded px-1.5 py-0.5 flex items-center h-6">
                      <span className="text-[10px] font-medium text-zinc-600 pr-3 border-r border-[#dadada]">
                        Calibri
                      </span>
                      <input
                        type="number"
                        ...
                        onChange={(e) => {
                          ...
                          if (selectedId)
                            setAnnotations((prev) =>
                              prev.map((a) =>
                                a.id === selectedId ? { ...a, fontSize: v } : a,
                              ),
                            );
                        }}
                  ...
                  <button
                    ...
                    className={cn(
                      "px-4 py-1 rounded flex items-center gap-2 transition-all w-full justify-center h-6",
                      activeTool === "text"
                        ? "bg-white shadow-md ring-2 ring-[#2b579a]"
                        : "hover:bg-white/60",
                    )}
              ...
              <div className="flex flex-col items-center h-full">
                <div className="flex-1 flex items-center gap-1">
                  <button
                    ...
                    disabled={!selectedId}
                    className="p-2 rounded hover:bg-red-50 disabled:opacity-20 flex flex-col items-center gap-1 w-12 transition-colors group"
                  >
                    <Trash2
                      size={18}
                  ...
                  <button
                    onClick={() => window.print()}
                    className="p-2 rounded hover:bg-white/60 flex flex-col items-center gap-1 w-12 transition-colors"
                  >
                    <Printer
                      size={22}
                      className="!text-[#2b579a]"
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        brushColor === pen.color &&
                          brushThickness === pen.size &&
                          ((pen.type === "highlight" &&
                            activeTool === "highlight") ||
                            (pen.type !== "highlight" && activeTool === "draw"))
                          ? "border-[#2b579a] ring-1 ring-[#2b579a]/40 shadow-md bg-blue-50/30 scale-105"
                          : "border-[#dadada] hover:border-[#2b579a]/50",
                      )}
                    >
                      <div
                        className="w-1.5 h-6 rounded-full mb-1 shadow-sm"
                        style={{
                          backgroundColor: pen.color,
                          opacity: pen.type === "highlight" ? 0.6 : 1,
                        }}
                      />
            ...
            <div className="flex h-full py-2 items-center gap-4">
              ...
              <div className="flex flex-col items-center h-full">
                <button
                  ...
                  className={cn(
                    "p-2 rounded hover:bg-white/60 transition-all flex flex-col items-center gap-1 w-16",
                    activeTool === "sign" &&
                      "bg-white shadow-sm ring-1 ring-[#dadada]",
                  )}
                >
                  <Signature
                    size={24}
              ...
              <div className="flex flex-col items-center h-full">
                <div className="flex-1 flex items-center gap-4">
                  <button
                    ...
                    className="p-2 hover:bg-white/60 rounded border border-[#dadada] !text-blue-900"
                  >
                    <ChevronLeft size={20} strokeWidth={3} />
                  </button>
                  <span className="text-[10px] font-black !text-zinc-950 bg-white px-3 py-1 rounded-sm border border-[#dadada]">
            ...
            {Array.from({ length: totalPages || 0 }).map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  currentPage === i + 1
                    ? "border-[#2b579a] ring-2 ring-[#2b579a]"
                    : "border-[#dadada] hover:border-[#2b579a]",
                )}
              >
                <span className="text-[10px] !text-zinc-950 font-black">
                  {i + 1}
                </span>
            ...
            {isLoading ? (
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  totalPages
                    ? "shadow-[0_0_20px_rgba(0,0,0,0.2),0_0_5px_rgba(0,0,0,0.1)] border border-[#dadada]"
                    : "w-[400px] h-[500px] flex flex-col items-center justify-center rounded-sm bg-white border border-[#dadada] border-dashed",
                )}
                style={{
                      ...
                      selectedAnnotation.type === "text" && (
                        ...
                        >
                          <input
                            ...
                            style={{
                              fontSize: `${selectedAnnotation.fontSize}px`,
                            }}
      ...
      {/* Embedded Toolbar (Modern) */}
      {isEmbedded && (
        <div className="h-14 border-b border-white/5 bg-[#12122b]/50 backdrop-blur-xl flex items-center justify-between px-6 shrink-0 z-50">
          <div className="flex items-center gap-1">
            {tools.map((tool) => (
              <button
                key={tool.id}
              ...
              >
                ...
                <span
                  className={cn(
                    "text-[10px] font-bold uppercase tracking-tight hidden lg:inline",
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-black/20 rounded-lg p-1">
              ...
              <span className="text-[10px] font-mono font-bold w-10 text-center text-slate-300">
                {zoom}%
              </span>
              <button
        ...
        <div className="h-6 bg-[#2b579a] text-white px-4 flex items-center justify-between text-[10px] no-print">
          ...
          <div className="flex items-center gap-6 text-white font-bold">
            ...
            <div className="w-24 h-[2px] bg-white/30 relative">
              <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-3 bg-white shadow-2xl rounded-[1px] border-2 border-[#1a478a]"
                style={{ left: `${(zoom / 300) * 100}%` }}
              />
            </div>
            <button
